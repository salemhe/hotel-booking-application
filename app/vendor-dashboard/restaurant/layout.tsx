...
function BranchDropdown() {
  ...
  const fetchBranches = () => {
    if (typeof window !== "undefined") {
      const stored = localStorage.getItem("adminBranches");
      if (stored) {
        try {
          const parsed = JSON.parse(stored);
          setBranches(parsed);
          if (!selected && parsed.length > 0) setSelected(parsed[0].id);
        } catch {}
      } else {
        setBranches([]);
      }
    ...
    <div className="mb-4">
      <div className="relative">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
        >
          <span>{selectedBranch ? selectedBranch.name : "No Branches"}</span>
        ...
        {open && branches.length > 0 && (
          <div className="absolute left-0 mt-2 w-full bg-white text-gray-900 rounded shadow z-20">
            {branches.map((branch) => (
              <div
                key={branch.id}
                className={`px-4 py-2 cursor-pointer hover:bg-teal-100 ${selected === branch.id ? "bg-teal-600 text-white" : ""}`}
