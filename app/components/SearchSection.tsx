    ...
    <div className="bg-white z-50 sm:absolute top-12 w-[90%] mx-auto left-0 right-0 rounded-2xl sm:rounded-full shadow-lg p-4 sm:p-2 justify-center mb-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div className="flex flex-col justify-center border-b sm:border-b-0 sm:border-r border-gray-200 pb-4 sm:pb-0 sm:pr-4">
          <label className="text-xs text-text-secondary text-left mb-1">Date</label>
        ...
        {/* Guests */}
        <div className="flex flex-col justify-center border-b sm:border-b-0 sm:border-r border-gray-200 pb-4 sm:pb-0 sm:pr-4">
          <label className="text-xs text-text-secondary text-left mb-1">Guests</label>
          <GuestDropdown onChange={counts => console.log('guest counts', counts)} />
        </div>
        {/* Search button */}
        <div className="flex items-center justify-center sm:justify-end w-full">
          <button
           className={`flex items-center gap-2 cursor-pointer text-white rounded-full px-6 py-3 transition ${activeTab === "restaurants" ? "bg-gradient-to-b from-[#0A6C6D] to-[#08577C] hover:from-[#084F4F] hover:to-[#064E5C] text-white rounded-full px-6 py-3 transition" : "bg-gradient-to-b from-blue-800 to-violet-500 hover:from-blue-900 hover:to-violet-600 text-white rounded-full px-6 py-3 transition"}`}>
...
export const SearchSectionTwo = () => {
  const [restaurant, setRestaurant] = useState<string>("International Restaurant");
  const [date, setDate] = useState<Date | null>(new Date("2025-05-23T00:00:00"));
  const [time, setTime] = useState<string | null>("7:30 pm");
  const [guests, setGuests] = useState<{ adults: number; children: number; infants: number }>({ adults: 2, children: 0, infants: 0 });
  ...
  return (
    <div className="mx-auto w-full max-w-3xl h-16 bg-white rounded-full shadow-lg flex items-center px-2 sm:px-4 gap-2 sm:gap-0">
      ...
      <div className="flex flex-col justify-center h-full px-4 border-r border-gray-200 min-w-0 w-1/4">
        <label className="text-xs text-text-secondary text-left mb-1">Restaurant/Cuisine</label>
        <input
          type="text"
          value={restaurant}
      ...
      <div className="flex flex-col justify-center h-full px-4 border-r border-gray-200 min-w-0 w-1/4 relative">
        ...
        {/* {showDate && (
          <div className="absolute top-14 left-0 z-50"> */}
            <DateDropdown selectedDate={date} onChange={d => { setDate(d); setShowDate(false); }} />
          {/* </div> */}
        {/* )} */}
      </div>
      {/* Time */}
      <div className="flex flex-col justify-center h-full px-4 border-r border-gray-200 min-w-0 w-1/4 relative">
        <label className="text-xs text-text-secondary text-left mb-1">Time</label>
        ...
        {/* {showTime && (
          <div className="absolute top-14 left-0 z-50"> */}
            <TimeDropdown selectedTime={time} onChange={t => { setTime(t); setShowTime(false); }} />
          {/* </div>
        )} */}
      </div>
      {/* Guests */}
      <div className="flex flex-col justify-center h-full px-4 border-r border-gray-200 min-w-0 w-1/4 relative">
        ...
        {/* {showGuests && ( */}
          {/* <div className="absolute top-14 left-0 z-50"> */}
            <GuestDropdown onChange={counts => { setGuests(counts); setShowGuests(false); }} />
          {/* </div>
        )} */}
      </div>
      {/* Search Button */}
