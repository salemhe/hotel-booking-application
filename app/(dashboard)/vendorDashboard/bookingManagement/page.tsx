...
import { Search, Trash2, Filter, ChevronDown, ChevronLeft, ChevronRight, Edit, X, Check } from 'lucide-react';
...
type StatusFilterType = 'all' | 'confirmed' | 'pending' | 'cancelled' | 'completed';
...
export default function AdminDashboard(): ReactElement {
  ...
  const BASE_URL: string = 'https://hotel-booking-app-backend-30q1.onrender.com';
  ...
  const fetchBookings = useCallback(async (type?: string, vendorId?: string): Promise<void> => {
    try {
      setLoading(true);
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch bookings. Please try again.';
  ...
  const showNotification = useCallback((message: string, type: 'success' | 'error' = 'success'): void => {
    if (!isMounted) return;
    
    const notification = document.createElement('div');
    notification.className = `fixed top-4 right-4 ${type === 'success' ? 'bg-green-500' : 'bg-red-500'} text-white p-4 rounded-md shadow-lg z-50 flex items-center`;
  ...
  useEffect(() => {
    ...
    try {
      ...
      socket.on('newBooking', (newBooking: Booking) => {
        ...
        // Show notification
        showNotification(`New booking #${newBooking._id.substring(0, 4)} received!`, 'success');
      });
      ...
      socket.on('bookingDeleted', (deletedId: string) => {
        setBookings(prevBookings => 
  ...
  const filteredBookings = sortBookings(bookings.filter(booking => {
    ...
    const searchMatches = 
      ...
      (booking.tableNumber && booking.tableNumber.toString().includes(searchQuery)) ||
      (booking.roomNumber && booking.roomNumber.toString().includes(searchQuery));
  ...
  const currentBookings: Booking[] = filteredBookings.slice(indexOfFirstItem, indexOfLastItem);
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-6 flex flex-col items-center justify-center">
        <div className="bg-white p-8 rounded-lg shadow-md max-w-md w-full">
          ...
          <p className="text-gray-700 mb-6">Unable to authenticate. Please log in again.</p>
          <button 
  ...
  return (
    ...
    <div className="max-w-7xl mx-auto">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger className="flex items-center gap-1 border border-gray-300 bg-white px-3 py-2 rounded-md">
              Sort by: {sortBy === 'newest' ? 'Newest' : 
                      ...
                      sortBy === 'guestsHigh' ? 'Most Guests' : 'Fewest Guests'}
              <ChevronDown className="h-4 w-4" />
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              <DropdownMenuItem onClick={() => setSortBy('newest')}>Newest</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSortBy('oldest')}>Oldest</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSortBy('guestsHigh')}>Most Guests</DropdownMenuItem>
              <DropdownMenuItem onClick={() => setSortBy('guestsLow')}>Fewest Guests</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
      ...
      <div className="flex flex-wrap gap-4 mb-4">
        ...
        <Select value={typeFilter} onValueChange={(value: BookingType) => setTypeFilter(value)}>
          <SelectTrigger className="w-[180px]">
              ...
              {currentBookings.length > 0 ? (
                currentBookings.map((booking) => (
                  <TableRow key={booking._id}>
                    <TableCell className="font-medium">{formatBookingId(booking._id)}</TableCell>
                    ...
                    <TableCell>
                      {editingBooking === booking._id ? (
                        <input
                          ...
                          value={booking.type === 'hotel' 
                            ? editForm.roomNumber || booking.roomNumber || '' 
                            : editForm.tableNumber || booking.tableNumber || ''}
                          onChange={(e) => {
                            const value = parseInt(e.target.value);
                            if (booking.type === 'hotel') {
                              setEditForm({...editForm, roomNumber: value});
                            } else {
                              setEditForm({...editForm, tableNumber: value});
                            }
                          }}
                        />
                      ) : (
                    ...
                    <TableCell>
                      {editingBooking === booking._id ? (
                        <Select 
                          ...
                          onValueChange={(value: 'confirmed' | 'pending' | 'cancelled' | 'completed') => setEditForm({...editForm, status: value})}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          </SelectTrigger>
        ...
        <div className="px-6 py-3 border-t border-gray-200 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2 text-sm">
            <span className="text-gray-700">
              {filteredBookings.length > 0 ? `${indexOfFirstItem + 1}-${Math.min(indexOfLastItem, filteredBookings.length)} of ${filteredBookings.length}` : '0-0 of 0'}
            </span>
